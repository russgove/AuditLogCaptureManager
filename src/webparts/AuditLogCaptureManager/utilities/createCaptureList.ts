...
export async function createCaptureList(client: AadHttpClient, siteUrl: string, listName: string, managementApiUrl: string): Promise<any> {
    ...
    try {
        ...
        await newList.list.fields.add("WebId", "SP.FieldText", { Description: "", FieldTypeKind: 3, Group: "Audit Capture" });
        await newList.list.fields.add("ListItemUniqueId", "SP.FieldText", { Description: "", FieldTypeKind: 3, Group: "Audit Capture" });
        await newList.list.fields.addNumber("RecordType"),// null, null, { Description: "The type of operation indicated by the record. See the AuditLogRecordType table for details on the types of audit log records.", FieldTypeKind: 3, Group: "Audit Capture" });
        ...
        await newList.list.fields.addNumber("UserType"), // { Description: "The type of user that performed the operation. See the UserType table for details on the types of users.", FieldTypeKind: 3, Group: "Audit Capture" });
            await newList.list.fields.addNumber("Version"),// null, null, { Description: "Version", FieldTypeKind: 3, Group: "Audit Capture" });
